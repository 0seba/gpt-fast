...
# from flash_attn import flash_attn_with_kvcache, flash_attn_triton_amd
...
class KVCache(nn.Module):
    def __init__(
        self, max_batch_size, max_seq_length, n_heads, head_dim, dtype=torch.float16
    ):
        super().__init__()
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # print(mask.to_dense())
        # y = torch.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=mask.to_dense().to(q.dtype), enable_gqa=False)
        ...
        #     k,
        #     v,
        #     cache_seqlens=input_pos[[0]],
        #     causal=True,
        #     # cache_seqlens=input_pos[-1],
        #     # causal=True,
        # )

        # ys = []
        # q = torch.chunk(q, self.n_local_heads, dim=1)
        # for i in range(self.n_local_heads):
        #     norm = torch.sum(torch.abs(q[i] * k[:, [i]]), dim=-1, keepdim=True)
        #     # print(norm)
